...
- **SDL2 / native library**: The release workflow publishes with `IncludeNativeLibrariesForSelfExtract=false` so SDL2.dll is placed alongside the exe on disk (Silk.NET finds it via P/Invoke). Setting this to `true` embeds SDL2.dll inside the single-file exe, which breaks runtime loading. The installer harvests the full publish directory, so SDL2.dll is included in the MSI automatically.
